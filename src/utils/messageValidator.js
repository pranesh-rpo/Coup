...
export function validateMessageContent(message, options = {}) {
  ...
  const spamPatterns = [
    /(.)\1{20,}/, // Repeated characters (more than 20)
  ...
  for (const pattern of spamPatterns) {
    if (pattern.test(messageText)) {
      logger.logError('MESSAGE_VALIDATION', null, new Error('Spam pattern detected'), 
        `Spam pattern detected in message: ${pattern}`);
      return { valid: false, error: 'Message contains patterns that may be flagged as spam' };
    }
  }
